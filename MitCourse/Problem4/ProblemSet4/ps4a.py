...
def getWordScore(word, n):
    ...
    counter = 0
    for e in word:
        ret += SCRABBLE_LETTER_VALUES[e] * len(word)
        counter += 1
    if counter == n:
        ret += 50
    return ret
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while sum(hand.values()) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(uInput, hand, wordList):
                # Reject invalid word (print a message followed by a blank line)
                print('Invalid word, please try again.')
            # Otherwise (the word is valid):
            else:
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                curScore = getWordScore(uInput, handSize)
                score += curScore
                print('\ "' + str(uInput) + ' \" earned ' + str(curScore) + ' points. Total: ' + str(score) + ' points')
                # Update the hand
    ...
    while True:
        ...
        if uInput == 'r':
            if previousHand:
                ...
                handSize = 0
                for elem in currentHand:
                    handSize += currentHand[elem]
                playHand(currentHand, wordList, handSize)
            else:
